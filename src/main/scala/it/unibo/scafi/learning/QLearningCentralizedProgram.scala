...
trait QLearningCentralizedProgram extends AggregateProgramWithLearning {
  ...
  def learn(algorithm: LearningBasedAlgorithm)(input: => INPUT)(initial: (OUTPUT, ACTION, STATE))(time: Double): OUTPUT = {
    ...
    val (programOutput, _, _) = rep((o0, a0, s0)) {
      case (aggregateResult, actionT, stateT) =>
        //println(s"o: $o, a: $a")
        //qLearning.setState(stateT) todo! the state evolution in perceived globally. If the evolution is evaluated locally, the behaviour seems that doesn't converge...in the centralized version the state change
